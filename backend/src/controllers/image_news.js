...
import { Updateimage_newsSchema, image_newsSchema } from '../Schema/product.js';

import Image_news from '../models/image_news.js';
...
export const update = async (req, res) => {
	try {
		// Validate
		const { error } = Updateimage_newsSchema.validate(req.body, {
			abortEarly: false,
		});
		if (error) {
			return res.status(400).json({
